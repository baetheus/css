...
Implement all nullpub utility classes using the vanilla-extract overlay. This
creates a complete utility-first CSS library with responsive variants.
...
```typescript
import { type Breakpoint, BREAKPOINTS } from "./config.js";
  ...
  for (
    ...
    for (const [scale, value] of Object.entries(SPACING_SCALE)) {
      ...
      for (const prop of props) {
        style[
          `${property}${prop.charAt(0).toUpperCase()}${
            prop.slice(1)
...
export const fontSize = styleVariants(
  withResponsive(
    Object.fromEntries(
      Object.entries(FONT_SIZE_SCALE).map((
        [key, value],
      ) => [key, { fontSize: value }]),
    ),
  ),
...
export const fontWeight = styleVariants(
  Object.fromEntries(
    Object.entries(FONT_WEIGHT_SCALE).map((
      [key, value],
...
```typescript
import { styleVariants } from "../vanilla/index.js";
import { type Direction, DIRECTION_MAP } from "./config.js";
...
```typescript
import { globalStyle, style } from "../vanilla/index.js";
...
```typescript
import { styleVariants } from "../vanilla/index.js";
import { type Direction, DIRECTION_MAP, SPACING_SCALE } from "./config.js";
...
function createBorderWidthStyles() {
  ...
  for (
    ...
    for (const [scale, value] of Object.entries(SPACING_SCALE)) {
      ...
      for (const prop of props) {
        style[
          `border${prop.charAt(0).toUpperCase()}${
            prop.slice(1)
          }Width` as keyof StyleObject
        ] = value;
      }
...
export { height, maxWidth, minHeight, width } from "./dimensions.js";

// Visual
export { color } from "./colors.js";
export { borderRadius, borderStyle, borderWidth } from "./border.js";
...
export { atBreakpoint, withResponsive } from "./responsive.js";
...
1. Run
   `deno eval "import { generateThemeCss } from './src/theme/index.ts'; Deno.writeTextFileSync('dist/theme.css', generateThemeCss());"`
   to generate the full CSS
